...
1. Usa un progetto **Free dedicato a SN**, in regione UE. Non applicare questa migrazione ai database degli altri progetti. Copia `.env.example` in `.env.local` e compila URL, chiave pubblica, chiave segreta server, `APP_ORIGIN`, contatto privacy e segreto manutenzione. Prima di provare la federazione, genera `FEDERATION_KEY_SECRET` con `openssl rand -base64 32`: perderlo rende inutilizzabili le chiavi private federate già archiviate.
