...
Cerchie, eventi, digest, album collaborativi, reazioni, menzioni, condivisioni interne e scoperta intenzionale sono domini futuri, non tabelle o API già disponibili. Le cerchie devono essere il primo blocco: membership privata, inviti accettati e ruoli protetti da RLS; eventi e album dipendono da quel perimetro di visibilità. Digest e scoperta devono usare query server-side spiegabili, senza profili comportamentali. Collaborazioni, reazioni e menzioni richiedono vincoli idempotenti e devono rispettare blocchi, rimozioni, export e cancellazione account. Ogni dominio va consegnato con migrazione, API, demo offline e test, senza anticipare strutture parziali nello schema di produzione.
