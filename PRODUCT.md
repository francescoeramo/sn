...
- **Federazione ActivityPub:** l’anteprima locale richiede un opt-in separato e revocabile, poi espone WebFinger, Actor con chiave pubblica, outbox, Note/Create testuali e i relativi allegati per i profili pubblici. Il percorso privato nel bucket e la chiave RSA privata cifrata non vengono pubblicati. Rendere privato il profilo revoca l’opt-in. Le richieste `Follow` e `Undo` in ingresso richiedono firma, digest e data validi; un worker disattivato per impostazione predefinita firma gli `Accept` e applica retry limitati. Restano il collaudo delle consegne, le altre attività e l’interoperabilità effettiva con Mastodon/Pixelfed. I profili privati non devono essere esposti come attori pubblici federabili.
