...
- **Federazione ActivityPub:** l’anteprima locale richiede un opt-in separato e revocabile, poi espone WebFinger, Actor, outbox, Note/Create testuali e i relativi allegati per i profili pubblici. Il percorso privato nel bucket non viene pubblicato. Rendere privato il profilo revoca l’opt-in. Restano da implementare firme HTTP, inbox, consegna e interoperabilità effettiva con Mastodon/Pixelfed. Tradurre post, like e follow in attività `Create`, `Like`, `Follow`, `Accept` e `Reject` e gestire le attività ricevute. I profili privati non devono essere esposti come attori pubblici federabili.
